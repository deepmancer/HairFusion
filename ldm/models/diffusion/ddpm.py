...
from ldm.modules.losses.perceptual import LPIPSWithoutDiscriminator, VGGLoss
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if self.use_u_cond_cnet:
            print("learnable matrix gene")
            self.learnable_matrix = nn.Parameter(torch.randn((1, self.control_stage_config.params.cond_first_ch, self.img_H//8, self.img_W//8)), requires_grad=True)
        else:
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        # pbe negative condition
            ...
            if self.use_u_cond_cnet:
                self.u_cond_cnet_prop = random.uniform(0,1)
                if self.u_cond_cnet_prop < self.u_cond_cnet_percent:
                    c["c_concat"] = [self.learnable_matrix.repeat(x.shape[0],1,1,1)]                   
            return self.p_losses(x, c, t, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if cond_output_dict is not None:
            cond_input = cond_output_dict["cond_input"]
            cond_output = cond_output_dict["cond_output"]
            if self.use_noisy_cond:
                cond_target = cond_output_dict["cond_noise"]
                loss_cond_simple = self.get_loss(cond_output, cond_target, mean=False).mean([1,2,3])
                loss_dict.update({f"cond_simple" : loss_cond_simple.mean()})
            else:
                cond_target = cond_input
                loss_cond_recon = LPIPSWithoutDiscriminator()(cond_output, cond_target)
                loss_dict.update({f"cond_recon" : loss_cond_recon})
                raise NotImplementedError  # autoencoder
        ...
        if cond_output_dict is not None:
            if self.use_noisy_cond:
                loss += self.l_cond_simple_weight * loss_cond_simple.mean()
            else:
                loss += self.l_cond_recon_weight * loss_cond_recon.mean()
